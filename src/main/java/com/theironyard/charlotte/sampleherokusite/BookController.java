...
//        books.add(greenEggsAndHam);
    ...
    @CrossOrigin
    @RequestMapping(path = "/checking_out/{index}", method = RequestMethod.PATCH)
    public void checkOut(@PathVariable("index") int index) {
        if (books.get(index).isCheckedOut() == false) {
            books.get(index).setCheckedOut(true);
        }
    }

    @CrossOrigin
    @RequestMapping(path="/checking_in/{index}", method = RequestMethod.PATCH)
    public void checkIn(@PathVariable("index") int index) {
        if(books.get(index).isCheckedOut() == true); {
